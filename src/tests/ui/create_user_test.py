...
class TestCreateUser:
    ...
    @pytest.mark.admin_session
    @pytest.mark.check_all_users_change(delta=0, username_source="invalid_user_request.username", should_exist=False)
    def test_admin_cannot_create_user_with_invalid_data(
        self,
        page: Page,
        api_manager: ApiManager,
        invalid_user_request: CreateUserRequest,
    ):
        AdminPanel(page).open() \
        .check_page_is_visible() \
        .check_alert_message_and_accept(BankAlert.USERNAME_MUST_BE_BETWEEN_3_AND_15_CHARACTERS) \
        .create_user(invalid_user_request.username, invalid_user_request.password) \
        .check_user_is_visible(invalid_user_request.username)
