...
class Condition:
    ...
    def and_(*conditions: Condition) -> Condition:
        ...
        sql = " AND ".join(f"({c.sql})" for c in conds)
        params: tuple[Any, ...] = tuple(p for c in conds for p in c.params)
...
class DBRequestBuilder:
    def __init__(self):
        ...
        self._where: Condition | None = None
    ...
    def extract_optional_as(self, dao_cls: type[T]) -> T | None:
        if self._request_type != RequestType.SELECT:
            raise NotImplementedError(f"Request type not supported: {self._request_type}")
        if not self._table:
            raise ValueError("Table is required")
